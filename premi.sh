...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "\033[96;1m                WELCOME TO SCRIPT CAPMODZ              \033[0m"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/sctunnel/capzvip/main/izin | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/sctunnel/capzvip/main/izin | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/sctunnel/capzvip/main/izin | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/sctunnel/capzvip/main/"
...
function send_notif() {
	...
	today=`date -d "0 days" +"%Y-%m-%d"`
	valid=$(curl https://raw.githubusercontent.com/sctunnel/capzvip/main/izin  | grep $ipsaya | awk '{print $3}')
	d1=$(date -d "$valid" +%s)
	d2=$(date -d "$today" +%s)
	certifacate=$(((d1 - d2) / 86400))

USRSC=$(wget -qO- https://raw.githubusercontent.com/sctunnel/capzvip/main/izin  | grep $ipsaya | awk '{print $2}')
EXPSC=$(wget -qO- https://raw.githubusercontent.com/sctunnel/capzvip/main/izin  | grep $ipsaya | awk '{print $3}')
...
<code>Aktif     : </code><code>$certifacate Hari</code>
...
<code>Pass    : </code>
<code>────────────────────</code>
<b> CAPMODZ STORE SCRIPT  </b>
<code>────────────────────</code>
<i>Automatic Notifications From Github</i>
"'&reply_markup={"inline_keyboard":[[{"text":"ᴏʀᴅᴇʀ","url":"https://wa.me/6285922180574"}]]}' 
...
wget raw.githubusercontent.com/sctunnel/capzvip/main/limit/limit.sh && chmod +x limit.sh && ./limit.sh
...
print_success "UDP-CUSTOM BY CAPMODZ STORE"
}
